...
3. **品質**：在掃描版 ResNet benchmark 的全部 4 個 baseline 失敗點上都辨識正確；TSMC 中文財報實測 19 組關鍵數字的數值全數正確，繁體中文一般文字樣本與原文文字層逐字一致

已知弱點：頁首大字距的裝飾性標題（公司名、報表標題）會被誤讀或幻覺（見中文財報一節）。此弱點與表格數字驗證無關，但排除了把 surya 直接當替代引擎的想法，符合本輪「不更換主 OCR 引擎」的範圍設定。
...
以本機一份繁體中文法律文件（有完好文字層可對照）第 4 頁測試：surya 輸出與原文文字層逐字一致，並正確重排欄內斷行、標出粗體標題。

### TSMC 中文財報（零文字層報表頁，2026-08-18 補測）

源 PDF 自 IR 網站重新下載（直接 URL 與 sha256 已記入 ground truth）。新檔 61 頁，報表頁完全沒有文字層（整頁掃描圖），與 4 月版描述的「CID 字型亂碼文字層」不同，但同樣必須走視覺 OCR 路徑。

對第 4 至 9 頁（合併資產負債表、合併綜合損益表）跑全頁辨識（6 頁共 215 秒），對照中文 ground truth：

- **19 組關鍵數字的數值全部正確**（含千分位逗號還原後逐值比對），沒有任何小數點或位數錯誤
- 表格結構正確：欄位跨列合併（rowspan/colspan）、雙欄期間表頭都恢復
- 失敗 3 項全在文字層：兩個報表標題（頁首大字距排版被讀壞或跳過）、「資產總計」標籤（讀出「資 產 總 計」帶字間空格，屬比對正規化問題，非辨識錯誤）
- 頁首裝飾性文字有幻覺案例：公司名被讀成「台灣積體電腦交通信訊電訊公司」、日期後多出「(星期日)」、「現金流量表」讀成「現金流匯表」

對照組：baseline（opendataloader-pdf force-OCR）在同一份 ground truth 為 12/26。粗略換算到本次 21 項檢核（2 標題加 19 關鍵數字），surya 為 18/21，且所有失分都與數字無關。結論：surya 適合做表格數字驗證層，不適合直接當全文轉換引擎。
...
1. 解 PRD Feature 1 兩個待決問題（驗證範圍、驗收標準）後開始實作驗證層
2. deepseek-ocr.rs 與 Qwen2-VL-OCR-2B 不需實測，surya 在中英文表格數字上均已達標
3. 驗證層比對時需正規化 CJK 字間空格（「資 產 總 計」），避免標籤比對誤報
